...
def save_item(record: dict) -> None:
    ...
    Writes the current schema: two-voice reflections + two-field preflections
    (charter_summary / judgemental). Legacy preflection / preflection_1p /
    neutral / idealisation columns are no longer written — they remain in the
    schema so historical rows stay intact, but new rows leave them NULL.
    """
    conn = _get_conn()
    conn.execute(
